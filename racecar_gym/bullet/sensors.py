...
class Lidar(BulletSensor[NDArray[Any, Float]]):
    @dataclass
    class Config:
        accuracy: float
        rays: int
    ...
    def observe(self) -> NDArray[Any, Float]:
        ...
        hit_fractions = np.array(results, dtype=object)[:, 2].astype(dtype=np.float32)
        ranges = self._config.range * hit_fractions + self._config.min_range
        noise = np.random.uniform(1.0 - self._config.accuracy, 1.0 + self._config.accuracy, size=ranges.shape).astype(np.float32)
...
class RGBCamera(BulletSensor[NDArray[Shape["*, *, 3"], Int]]):
    @dataclass
    class Config:
        width: int
        height: int
    ...
    def observe(self) -> NDArray[Shape["*, *, 3"], Int]:
...
class AccelerationSensor(BulletSensor[NDArray[Shape["6"], Float]]):
    @dataclass
    class Config:
        time_delta: float
    ...
    def observe(self) -> NDArray[Shape["6"], Float]:
...
class VelocitySensor(BulletSensor[NDArray[Shape["6"], Float]]):
    @dataclass
    class Config:
        gaussian_noise: float
...
class PoseSensor(BulletSensor[NDArray[Shape["6"], Float]]):
    @dataclass
    class Config:
